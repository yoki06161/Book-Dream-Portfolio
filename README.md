...
#### 3. 문제 해결 및 디버깅
- **채팅 이미지 전송 방식 개선**: 불안정한 이미지 직접 전송 로직을 제거하고, 서버에 먼저 업로드 후 URL을 전송하는 안정적인 방식으로 **변경했습니다.**
- **UI 버그 수정**: 이미지 메시지 수신 시, 비동기 로딩 문제로 스크롤 위치가 위로 올라가던 버그를 이미지의 `onload` 이벤트를 활용하여 **해결했습니다.**
